...
import { capitalize, createSlug, timeAgo, truncate } from "@/components/utils/miniUtils";
...
function ArticleListPage() {
  ...
  //         ↓
  // calls updateQuery with a callback
  //         ↓
  // updateQuery:
  //   1. copies current URL params
  //   2. hands copy to your callback
  //   3. callback modifies the copy
  ...
  //         ↓
  // statusParam = searchParams.getAll("status") re-reads
  //         ↓
  // filteredArticles recomputes
  //         ↓
  // UI updates
    ...
    <div className="h-full overflow-y-auto p-4 space-y-6">
      ...
      {!isLoading && filteredArticles.length === 0 && (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          <div
            className="w-16 h-16 bg-muted rounded-full flex
...
function ArticleCard({ article }: { article: any }) {
  ...
  const coverUrl = article.coverImage ? getFileUrl(article.coverImage) : null;
        ...
        {article.category && (
          <span
            className="absolute top-3 left-3
                           ...
                           px-2.5 py-1 rounded-full border border-white/10 capitalize"
          >
            {article.category}
          </span>
        )}
